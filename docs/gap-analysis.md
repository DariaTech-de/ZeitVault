...
| B-12 | teilweise | Schnitt 1 + Nachtrag (Basis, BL-6): Zeitdauern sind ganze Minuten; die Messung eines Intervalls wird genau EINMAL gerundet, Zeitscheiben leiten sich aus kumulierten Grenzen ab (Summeninvariante als Property-Test, `localtime.ts` sliceIntervalByLocalDay + `localtime.test.ts`); konfigurierbare Rundung setzt am EREIGNIS an (`stamping/rounding.ts`, Standard 'none', asymmetrische BV-Regeln abbildbar) | Offen: Rundungsmodus PRO MANDANT konfigurierbar + im Audit-Trail sichtbar (Schnitt 3); Betrags-/Lohnartenrundung erst am Ende (Schnitt 4). | M |
