...
|---|---|---|---|---|
| A-01 | teilweise | `apps/api/src/db/schema.ts:93-113` (time_entries: start_at/end_at timestamptz), `:134-158` (stamp_events); Dauer abgeleitet in `packages/domain/src/stamping/evaluate.ts:13-21` | Start/Ende als timestamptz, Dauer wird berechnet, nicht gespeichert ✓. Aber: zwei parallele Modelle (stamp_events + time_entries), und Nachtschichten über Mitternacht sind nicht erfassbar (409, siehe K-02) → „je Zeitscheibe" nur eingeschränkt erfüllt. | M |
...
| A-03 | fehlt | Gegenstelle: `packages/types/src/stamp.ts:19` (occurredAt beliebig rückdatierbar, ohne Begründung), kein Treffer für `late_entry` im Repo | Nacherfassung > 24 h ist heute unbegrenzt und UNMARKIERT möglich — weder Pflicht-`reason` noch `late_entry`-Kennzeichen. | S |
...
| B-12 | fehlt | — (kein Rundungskonzept in `packages/domain`; grep leer). Verwandt: `engine.ts:13-19` liefert Bruchminuten (Float, siehe Blocker 6) | Kein konfigurierbarer Rundungsmodus, nichts im Audit-Trail. | M |
...
| K-01 | widerspricht | Speicherung timestamptz ✓ (`schema.ts:141`), aber Bewertung nach UTC-Kalendertag: `reporting.service.ts:28-30,190`, `stamping.service.ts:31-40` (dayWhere UTC), `export.service.ts:225` | Es gibt keinen Zeitzonenbegriff (kein Einsatzort, keine TZ am Mandanten). „Bewertung in lokaler Zeitzone" ist strukturell nicht erfüllbar; DST-Tests fehlen. Die UTC-Tagesgrenze verschiebt zudem lokale Tage (23:30-Stempel MESZ landet am Vortag). | L |
| K-02 | widerspricht | `stamping.service.ts:96-108` (Kandidaten-Validierung im UTC-Tagesfenster) + `fold.ts:67-70` (clock_out bei state 'out' wirft) | Eine Nachtschicht über Mitternacht kann nicht einmal ERFASST werden: Ausstempeln am Folgetag → StampTransitionError → 409. Eine Monats-Zuordnungsregel (Schichtbeginn vs. Splittung) existiert folglich auch nicht. | L |
| K-03 | widerspricht | `reporting.service.ts:187-216` (aggregateDays foldet je UTC-Tag; verwaiste Folgetags-Events werfen) | Tagesscharfe Faltung bricht bei tagesübergreifenden Schichten (Report wirft bzw. bewertet falsch). Werktags- vs. Kalendertags-Lesart des § 3 ist nirgends abgebildet. | L |
