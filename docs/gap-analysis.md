...
| B-01 | implementiert | Schnitt 3: `arbzg/averaging.ts` (rollierendes Fenster, Monate ODER Wochen konfigurierbar; werktäglicher 8-h-Durchschnitt Mo–Sa); Report `GET /reports/averaging`. Tests: `averaging.test.ts` (AK: 10-h-Tage ohne Ausgleich → Verstoß; mit Ausgleich kein Befund), Service-Level `averaging.itest.ts` | Feiertagsbereinigung des Werktagsbegriffs folgt mit der C-08-Integration (dokumentiert). | — |
...
| B-03 | implementiert | Schichtübergreifende 11-h-Prüfung (Schnitt 1, AK) + Schnitt 3: 10-h-Verkürzung per TV-Regelsatz mit AUSGLEICHSPFLICHT (`arbzg/rest-compensation.ts`: ≥ 12-h-Ruhe binnen Monat/4 Wochen; Warnung vor, Verstoß nach Fristablauf; im Verstoßreport). Tests: `rest-compensation.test.ts` + `rest-compensation.itest.ts` | Ruhezeit-Anker ist der Schichtbeginn — auch vor unaufgelösten Folgeschichten sicher prüfbar (ADR-0019). | — |
| B-04 | implementiert | Schnitt 3: `night_worker`-Stammdatum (Migration 0023, § 2 Abs. 5; Pflege durch HR) + eigene kürzere Ausgleichsperiode (`nightWorkerAveragingPeriod*`, Gesetz 1 Monat). Test deckt die Abweichung zu B-01 ab (`averaging.test.ts`/`averaging.itest.ts`: überzogener Juni kippt nur für Nachtarbeitnehmer). | — |
| B-05 | implementiert | Schnitt 3: `arbzg/night.ts` — ArbZG-Nachtzeit (23–6; Bäcker 22–5 per TV-Regelsatz `arbzgNight*Minute`) getrennt vom festen EStG-Fenster (20–6). AK-Beweis-Test: 20:30 → `taxNightBonus=true`, `arbzgNightWork=false` (`night.test.ts`). | — |
| B-06 | implementiert | Schnitt 3: `arbzg/sunday-rest.ts` (Ersatzruhetag binnen 14 Tagen/8 Wochen mit Fristüberwachung: PENDING-Warnung vor, MISSING-Verstoß nach Fristablauf; 15-freie-Sonntage-Zählung je Jahr) + `GET /reports/sunday-rest?year=`. Tests: `sunday-rest.test.ts` + `sunday-rest.itest.ts`. Feiertage einsatzortscharf (Bundesland). | — |
| B-07 | implementiert | Schnitt 3: `JARBSCHG_2026_V1` (8 h/Tag UND 40 h/Woche, Pausen 30/60 min bei > 4,5/6 h, max. 4,5 h am Stück, 12 h Freizeit, Beschäftigungsfenster 06–20 Uhr) + `selectLawPackage` wählt die Gesetzes-Baseline je Bewertungstag über das optionale Geburtsdatum (Migration 0022, Zweckbindung dokumentiert) — Umschaltung am 18. Geburtstag automatisch. Tests: `minors.test.ts` + Ende-zu-Ende `minors.itest.ts` (Tag vor/am 18. Geburtstag). | — |
| B-08 | implementiert | Schnitt 2: `collective_agreements` + `rule_sets` (Migration 0020, RLS/FORCE); Referenzpflicht doppelt erzwungen (DB-CHECK `rule_sets_agreement_required` + Service-Validierung Existenz/aktiv/Ebene, `rules.service.ts`). Tests: `apps/api/test/rules.itest.ts` (RLS, „ohne Referenz nicht aktivierbar", falsche Ebene, Deaktivierungsschutz) | Abweichender Regelsatz ist ohne existierendes, aktives `collective_agreement`-Objekt nicht aktivierbar; BV kann ermächtigenden TV referenzieren (`based_on_id`). Kulanzfrist (ADR-0019) hängt als `openShiftGraceMinutes` an der Regelschicht. | — |
| B-09 | implementiert | Schnitt 2: `packages/domain/src/arbzg/layering.ts` (Reihenfolge Gesetz→TV→BV→individuell dokumentiert; Schutzrichtung je Parameter; Günstigkeitsprinzip; `RuleConflictError` bei gleicher Ebene/gleichem Parameter/verschiedenen Werten, individueller Verschlechterung und fehlender Referenz). Tests: `layering.test.ts` (9 Fälle) + Anlage-Probeauflösung im Service (409 statt vergifteter Bewertung, `rules.itest.ts`). Alle Bewertungspfade lösen je Abrechnungstag auf; Herkunft je Parameter via `GET /rules/effective`. | Konflikte werfen explizit — keine stille Priorisierung. | — |
| B-10 | teilweise | Schnitt 2: `rule_sets` mit `valid_from`/`valid_to` je Satz (DB, mandantenfähig); rückwirkende Anlage/Deaktivierung triggert Neubewertungs-Lauf mit Protokoll + Audit (`reprocessing.service.ts`, `reprocessing_runs`; Test `reprocessing.itest.ts`: Abschluss im Juli gültig ab Januar → Lauf über betroffene Perioden, Befunde ändern sich) | Offen: DIFFERENZ-Erzeugung gegenüber eingefrorenem Vorstand — braucht das Periodenmodell (F-04/F-03, Schnitt 5). | M |
| B-11 | implementiert | Schnitt 3: Mitarbeitergruppen (Migration 0021, RLS) + gruppen-gescopte Regelsätze; `maxWorkingTimeMode: daily|weekly` (tarifgebunden), Wochensummen IMMER parallel berechnet (`arbzg/weekly.ts`), Befunde je Modus. Tests: `weekly.test.ts` + `employee-groups.itest.ts` (Gruppe weekly: 55 h → Wochenverstoß, keine Tagesbefunde; außerhalb: Tagesmaßstab). | — |
| B-12 | implementiert | Schnitt 3: Rundungsmodi je Ereignisart als Regelparameter (`rounding*`, Standard immer 'none'; nur per BV-Regelsatz, individuell abgewiesen — mitbestimmungspflichtig); `stamp()`/Offline-Sync wenden den am Stempeltag wirksamen Modus AM EREIGNIS an; Audit-Payload trägt Modus + Roh-Zeitstempel. Summeninvariante als Property-Test (Schnitt-1-Nachtrag). Tests: `rounding-config.itest.ts`. Offen bleibt nur die Betrags-/Lohnartenrundung (Schnitt 4, C-Block). | — |
| B-13 | implementiert | Schnitt 3: Prüfung beim Erfassen ✓ (Findings in Antwort/Heute inkl. Live-Ruhezeit); Verstöße erzeugen sofort eine FK-Inbox-Warnung (`notifications`, Migration 0024, RLS; `GET /notifications` + gelesen-Markierung). Test: `notifications.itest.ts`. | „Planen" existiert nicht — Anbindung folgt mit der Schichtplanung (vermerkt). | — |
| C-01 | implementiert | Schnitt 4: `packages/domain/src/surcharge/estg3b.ts` (Klassifikation je MINUTE an echten Instants, Wanduhr via Einsatzort-Zeitzone) + Pipeline `apps/api/src/reporting/surcharge-report.service.ts` (`GET /reports/surcharges`: echte Stempel → Korrektur-Abschluss → Schichten). Tests: `estg3b.test.ts` (Splittung exakt an 20:00) + `surcharge-report.itest.ts` | Die alte spans-basierte Engine (`compute.ts`) bleibt für konfigurierbare TV-Sätze; gesetzliche §-3b-Fenster/Sätze sind bewusst NICHT konfigurierbar (ADR-0018). | — |
| C-02 | implementiert | Schnitt 4: 40-%-Fenster 0–4 Uhr NUR bei Arbeitsaufnahme vor 0 Uhr (`estg3b.ts`, `shiftStartAt < localDayStart`); night40/night25 partitionieren die Nacht (nie beide). AK-Tests wörtlich: 22–6 → 40 % für 0–4; 01–06 → nur 25 % (`estg3b.test.ts`) | — | — |
| C-03 | implementiert | Schnitt 4: Sonntagsklasse minutengenau am lokalen Datum (`estg3b.ts`); an echte Stempel angebunden (`surcharge-report.itest.ts`) | — | — |
| C-03a | implementiert | Schnitt 4: Fortwirkung 0–4 Uhr des Folgetags als EINE Mechanik mit C-02/C-04a (`carryoverOf`: Tagesklasse des Vortags um 23:59 wirkt bei Aufnahme vor 0 Uhr fort). AK-Tests wörtlich: So 22 – Mo 6 → 50 % bis Mo 04:00; Mo 01–06 → kein Sonntagszuschlag | — | — |
| C-04 | implementiert | Schnitt 4: Feiertag 125 % inkl. 31.12. ab 14:00 (Teiltagsbeginn, Minute 840); Fortwirkung 0–4 wie C-03a. Tests: `estg3b.test.ts` | Welche Tage Feiertage sind, entscheidet der Einsatzort (C-08). | — |
| C-04a | implementiert | Schnitt 4: mit C-02/C-03a als eine Fortwirkungs-Mechanik; AK-Test Feiertag 22:00 – Folgetag 06:00 → 125 % bis 04:00 | — | — |
| C-05 | implementiert | Schnitt 4: 150-%-Klasse für 24.12. ab 14:00, 25./26.12., 01.05. (`estg3b.ts`); Konkurrenz: 25.12. als Feiertag → NUR 150 % (höchster Satz). Tests: `estg3b.test.ts` | — | — |
| C-06 | implementiert | Schnitt 4: Geldmodell `packages/domain/src/money/money.ts` (Cent-Integer, nie Float; EINE kaufmännische Rundung je Lohnart+Periode); zwei getrennte Grundlohn-Kappungen 50 €/h (Steuer) und 25 €/h (SV) mit separaten Ausgabefeldern; `employees.hourly_base_wage_cents` (Migration 0025). AK-Test 40 €/h → steuerfrei, aber beitragspflichtig (`money.test.ts`, `surcharge-report.itest.ts`) | Ohne gesetzten Grundlohn werden nur Minuten ausgewiesen, keine Beträge. | — |
| C-07 | implementiert | Schnitt 4: Kumulationsregeln im `estg3b.ts`-Docblock dokumentiert + testabgedeckt: Nacht kumuliert mit genau EINER Tagesklasse; Tagesklassen konkurrieren (150 > 125 > 50, Feiertag verdrängt Sonntag); night40/night25 partitionieren die Nacht (`estg3b.test.ts`) | — | — |
| C-08 | implementiert | Schnitt 4: `holidaysForLocation`/`isHolidayAtLocation` (Bundesland + Gemeinde-Schlüssel `fronleichnam`/`mariae_himmelfahrt`/`friedensfest` am Einsatzort, `work_locations.municipal_holiday_keys`, Migration 0026; bewusst keine amtliche Gemeindeliste); Pipeline bewertet je SCHICHT gegen den aufgelösten Einsatzort. Tests: `holidays-location.test.ts`, `work-location-holidays.itest.ts`, `surcharge-report.itest.ts` (BY vs. SN je Schicht) | Spec-AK nennt Hessen irrig als Land ohne Fronleichnam — fachlich falsch, gemeldet (findings Nr. 11); Kalender bildet die echte Rechtslage ab. | — |
| C-09 | implementiert | Schnitt 4: `stamp_events.work_kind` (Migration 0027; nur am `clock_in`, Korrekturen erben): Vollarbeit/Bereitschaftsdienst/Rufbereitschaft/Reisezeit. ArbZG: Bereitschaftsdienst = ARBEITSZEIT, Rufbereitschaft = RUHEZEIT (zählt nicht, verschiebt den Ruhezeit-Anker nicht, eigener Zähler `standbyMinutes`), Reisezeit wie Vollarbeit (dokumentierter Default). Eigene Payroll-Kategorie → eigene Lohnart + Faktor via Mapping (C-11); Rufbereitschaft ohne §-3b-Zuschläge. Tests: `work-kind.test.ts`, `work-kind.itest.ts` | Web-/Mobile-Erfassung bietet die Art noch nicht zur Auswahl an (API/Sync tragen sie); UI nachziehen (findings Nr. 7-Umfeld). | — |
| C-10 | implementiert | Schnitt 4: `arbzg/overtime.ts` (`summarizeOvertime`): ÜBERSTUNDEN über das Vertragsmaß (`contractualWeeklyMinutes`, neutraler Regelparameter, pro TV konfigurierbar; 0 = nicht konfiguriert → Zähler null) vs. MEHRARBEIT über `maxWeeklyMinutes` — zwei getrennte Zähler je Kalenderwoche im Stundenzettel (`overtimeWeeks`). Tests: `overtime.test.ts`, `overtime.itest.ts` (50-h-Woche: ohne TV nur Mehrarbeit, mit 38-h-TV zusätzlich 12 h Überstunden) | — | — |
| C-11 | implementiert | Schnitt 4: `payroll_mappings` (Migration 0028, RLS/FORCE; Upsert je Kategorie, AuditEvents `payroll_mapping.set/delete`); Lohnexport lädt das persistierte Mapping (kein Request-Body mehr); Admin-Endpoints `GET/PUT/DELETE /exports/payroll-mapping` + Admin-UI-Seite Lohnarten (Bewertungsart → Lohnartennummer, Faktor). AK-Itest: Änderung ohne Deployment wirksam (`payroll-mapping.itest.ts`) | Faktor wird als eigene CSV-Spalte mitgeführt — Minuten bleiben echte Minuten. | — |
...
| E-09 | teilweise | Workflow mit Genehmiger + Audit: `absence.service.ts:62-100`, `transitions.ts`; RLS-Test `absence-rls.itest.ts` | Genehmigen/Ablehnen/Stornieren ✓. Es fehlen: Vertretungsregelung, Eskalation (inkl. Eskalation unaufgelöster Schichten an die FK, ADR-0019), Kopplung an den Monatsabschluss (Cut-off existiert nicht, F-03). | M |
...
| F-03 | fehlt | — (kein Freeze/Periodenabschluss in Schema oder Services) | Kein Periodenbegriff, keine Freigabekette, keine DB-Sperre für eingefrorene Zeiträume. Vorgriff (ADR-0019): Der Lohnexport schließt unaufgelöste Schichten bereits aus; der Freeze muss Perioden mit unaufgelösten Schichten blockieren. | L |
...
| G-04 | fehlt | — (kein Lücken-Report; reporting.service kennt nur Verstöße/Salden) | Report „Erfassungslücken > 7 Tage" fehlt (§ 17 MiLoG als Anker). Bausteine vorhanden: createdAt vs occurredAt sowie der `SHIFT_UNRESOLVED`-Befund je Abrechnungstag (ADR-0019). | S |
...
| K-01 | implementiert | Schnitt 1: Speicherung UTC ✓, Bewertung in der Einsatzort-Zeitzone (`packages/domain/src/localtime/localtime.ts`, `day-view.ts`; alle Services auf lokale Abrechnungstage umgestellt). DST-AK-Tests: `localtime.test.ts`/`shifts.test.ts` (Schicht 22:00–06:00 → 7 h bzw. 9 h) und Service-Level `stamping-shifts.itest.ts` gegen die echte DB. Nachtrag: Wanduhr-Eingaben nur über `instantFromLocal` (Frühjahrslücke → Fehler, Herbst-Doppelstunde → explizite Auflösung; `localtime.ts` + Tests). | — |
...
| K-04 | implementiert | Schnitt 4: Zuschlagssplittung minutengenau an ECHTEN Instants über Mitternacht und DST (`estg3b.ts`); reale Pipeline-Itests: DST-Frühjahrsnacht 28./29.03. (7 reale Stunden, 0–4-Fenster real 3 h), Prüfstein So 22 – Mo 06; Summeninvariante (jede Minute genau eine Nacht- und eine Tagesklasse) | — | — |
...
| K-06 | implementiert | Schema-Review: durchgängig timestamptz ✓. Schnitt 1: Einsatzort-Entität mit IANA-Zeitzone (ADR-0016; `work_locations` + Historie, Migration 0019, RLS-Test `work-location-rls.itest.ts`), Auflösung Übersteuerung > Zuordnung > Mandanten-Default (`work-location.service.ts` resolve; KEIN stiller Fallback — fehlender Default wirft, Default ist Pflicht-Stammdatum, Seed legt ihn an), Bewertung gegen die aufgelöste Zeitzone in Stempeln/Heute/Report/Export/Dashboard. | — |

Status-Zählung (79 Anforderungen; Stand nach Schnitt 4, 2026-07-09): implementiert 36 (B-02 aus Schnitt 0; A-03/K-01/K-03/K-06 aus Schnitt 1; B-08/B-09 aus Schnitt 2; B-01/B-03/B-04/B-05/B-06/B-07/B-11/B-12/B-13 aus Schnitt 3; C-01/C-02/C-03/C-03a/C-04/C-04a/C-05/C-06/C-07/C-08/C-09/C-10/C-11/K-04 aus Schnitt 4) · teilweise 22 (B-10 bleibt teilweise bis F-04) · fehlt 18 · widerspricht 0 · nicht auffindbar 3 (I-04, J-05, K-05). Nachtrag zur Schnitt-1-Abnahme (2026-07-08): Implicit-Close durch das unresolved-Zustandsmodell ersetzt (ADR-0019), Rundung ans Ereignis verlegt + Summeninvariante (B-12-Basis), Korrektur-Abschluss der Ladefenster, Einsatzort-Übersteuerung beim Stempeln validiert, Europe/Berlin-Fallback entfernt, `instantFromLocal` mit DST-Eingabevalidierung. Nebenbefunde: [`findings.md`](findings.md).
